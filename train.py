...
def get_class(class_path):
    files = os.listdir(class_path)  # 文件夹下所有目录的列表
    for f in files:
        if os.path.isdir(class_path + '/' + f):  # 这里是绝对路径，该句判断目录是否是文件夹
            dirList.append(f)
get_class('all/train')
class_names1 = {}
for i in range(len(dirList)):
    class_names1[dirList[i]] = i


cfg = edict({
    'data_path': 'all/train',  # 训练数据集，如果是zip文件需要解压
    'test_path': 'all/test',  # 测试数据集，如果是zip文件需要解压
    'data_size': 50000,
    'HEIGHT': 224,  # 图片高度
    'WIDTH': 224,  # 图片宽度
    '_R_MEAN': 123.68,
...
def read_data(path, config, usage="train"):
    # 从目录中读取图像的源数据集。
    dataset = ds.ImageFolderDataset(path, class_indexing=class_names1)
